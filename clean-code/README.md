...
> try/catch 블럭은 원래 추하다. 코드 구조에 혼란을 일으키며, 정상 동작과 오류 처리 동작을 뒤섞는다.<br/>
> 그러므로 try/catch 블럭을 별도 함수로 뽑아내는 편이 좋다.
...
> **닫는 괄호에 다는 주석**
> 때로는 프로그래머들이 닫는 괄호에 특수한 주석을 달아놓는다. 중첩이 심하고 장황한 함수라면 의미가 있을지도 모르지만 작고 캡슐화된 함수에는 잡음일 뿐이다.<br/>
